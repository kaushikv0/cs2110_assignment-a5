...
class Map {
	...
	private void initInitialSettings() {
		...
		randomFlowers    = true;
		expected_learning_flowers = 75;
		expected_running_flowers = 75;
		
		randomAromaIntensity    = true;
		default_aroma_intensity = 25.0;
		min_aroma_intensity     = 10.0;
		max_aroma_intensity     = 50.0;
	...
	private List<Flower> parseFlowers(String fields[], int row, int column, 
		...
		if (FLOWER_INDEX > fields.length - 1) {
			if (randomFlowers && rand.nextBoolean(probability)) {
				...
				learningFlowerPositions.add(flowerPos);
				return flowers;
			}
		...
		for (String nameSuffix : flowersStrings) {
			...
			learningFlowerPositions.add(position);
	...
	private int randomLight() {
		return rand.nextInt(min_light, max_light);
	}
	...
	private Wind randomWind() {
		int intensity = rand.nextInt(min_wind, max_wind);
		Direction direction = rand.nextElement(DIRECTIONS);
	...
	public void refreshState() {
		...
		butterfly.state = at(new Position(butterfly.location)).tileState;
	}

	/**
	 * Flies a butterfly. First, the game's turn is incremented. Then, a 
	 * collision is detected. If no collision occurs, the butterly is first
	...
	private int updateCosts(Position toPos, Speed speed) {
		...
		/* Wind Costs. TODO: comment this wizardry. */
